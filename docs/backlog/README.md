...
| [One undecodable member fails the whole package](undecodable-metadata-fails-the-whole-package.md) | sources | Degrading instead needs a reported-skip field, and reporting it honestly is a corpus schema change |
| [Upgrading to the generation layout strands the previous cache](upgrading-strands-the-previous-cache-layout.md) | server | One-time 1.45 GB per machine; deleting a directory the user did not ask about deserves a deliberate choice |
